...
function display(data)
{
    ...
    for(let i=0;i<data.length;i++)
    {
        ...
        msg += "<td>"+ row.price +"</td>";
        msg += "<td>"+ row.payment +"</td>";
        if(row.status=="In Progress")
        {
            msg += "<td><span class='status inprogress'>"+ row.status +"</span></td>";    
        }
        else{
            msg += "<td><span class='status "+row.status+"'>"+ row.status +"</span></td>";
        }
        msg += "<td> <a href='#' onClick=addToCart("+i+")><ion-icon name='cart-outline'></ion-icon></a> </td>";
        
        msg += "</tr>";
    }


    document.getElementById("product_table").innerHTML=msg;
}
