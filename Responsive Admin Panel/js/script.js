...
function checkCredential(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        let datas = data[i];
        if (datas.userEmail == useremail && datas.userPass == userpassword) {
...
function register() {
    ...
    if (userpassword == usercpassword) {
        ...
        let temp = {
            'id': totalusers + 1, 'userName': username, 'mobile': usermobile
            , 'city': usercity, 'userEmail': useremail, 'userPass': userpassword
        }
